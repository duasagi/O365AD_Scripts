...
# -----------------------------
$global:WorkNotesMessage = ""
$global:ErrorFlag = $true   # DEFAULT = true (because only SUCCESS should be false)
...
if ($action -eq "add") {
    ...
    if ($assignedSkus -contains $LicenseSkuId) {
        Set-Result -Message "License already assigned to user." -IsError $true
    }
...
if ($action -eq "remove") {
    ...
    if ($assignedSkus -notcontains $LicenseSkuId) {
        Set-Result -Message "License not assigned to user. Nothing to remove." -IsError $true
    }
